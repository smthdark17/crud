...
class SubjectTable
{
	...
	{
		$query = Connection::prepare('INSERT INTO `subject` (`id`, `name`)' .
			' VALUES (:id, :name)');
	...
	{
		$query = Connection::prepare('DELETE FROM `subject` WHERE `id` = :id');
		$query->bindValue(':id', $id, \PDO::PARAM_INT);
		if (!$query->execute()) {
			echo "sebya udali";
		}
	...
	{
		$query = Connection::prepare('UPDATE `subject` SET `name` = :name' . ' WHERE `id` = :id');
		$query->bindValue(':id', $id, \PDO::PARAM_INT);
		$query->bindValue(':name', $name, \PDO::PARAM_STR);
